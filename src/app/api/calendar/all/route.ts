...
import { generateEventsICS, convertEventToICS, DEFAULT_SERVICE_TIMES, generateServiceTimesICS, ICSEvent } from '@/lib/ics';
...
export async function GET(request: NextRequest) {
  try {
    ...
    icsEvents.forEach(event => {
      ...
      if (event.location) {
        ics.push(`LOCATION:${event.location.replace(/[\\;,\n\r]/g, (match) => '\\' + match)}`);
      }
    ...
    DEFAULT_SERVICE_TIMES.forEach(service => {
      ...
      if (service.description) {
        ics.push(`DESCRIPTION:${service.description.replace(/[\\;,\n\r]/g, (match) => '\\' + match)}`);
      }

      if (service.location) {
        ics.push(`LOCATION:${service.location.replace(/[\\;,\n\r]/g, (match) => '\\' + match)}`);
      }
    ...
    return new NextResponse(icsContent, {
      ...
      headers: {
        ...
        'Content-Disposition': 'attachment; filename="fmc-bethlehem-complete.ics"',
